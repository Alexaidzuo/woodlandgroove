...
onchange:function(){updateViewData("name",this.value)},label:__("View Name","formidable-views"),value:frmViewsEditorViewData.name}),getApplicationDropdown(),getShortcodeExample(),getPhpEchoExample(),separator(),getSettingsForActiveViewType(),getDisplaySettings(),getMetaboxes()];noEntriesTextareaWrapper=collapsible(div({text:__("No Entries Message","formidable-views")}),getNoEntriesTextarea());noEntriesTextareaWrapper.id="frm_no_entries_textarea_collapsible";children.push(noEntriesTextareaWrapper);
return children}function updateViewData(key,value){frmViewsEditorViewData[key]=value;preventUnsavedChanges()}function getViewKeyInput(){var output=wrapFormInput({id:"frm_views_view_key",onchange:function(){state.viewKey=this.value;preventUnsavedChanges()},label:__("View Key","formidable-views"),value:frmViewsEditorViewData.viewKey});output.style.margin="10px 0 0";return output}function getDetailSlugPlaceholder(){var placeholder=dataProcessor.placeholder("detailSlug",maybeGetDetailSlugInput);placeholder.id=
"frm_views_detail_slug_wrapper";return placeholder}function maybeGetDetailSlugInput(detailSlug){var wrappedInput;if(!viewIsDynamic())return emptyNode();wrappedInput=wrapFormInput({id:"frm_views_detail_slug",class:"frm_half",onchange:function(){updateDataProcessor("detailSlug",this.value);refreshLayoutBuilderArea()},label:__("Detail Page Slug","formidable-views"),value:detailSlug});if(isSingleEntryType()||isCalendarType())wrappedInput.style.marginTop=0;appendTooltipTriggerAferInputLabel(wrappedInput,
getDetailSlugInputTooltipText());wrappedInput.firstChild.classList.add("frm_grid_container");jQuery(wrappedInput.querySelector("#frm_views_detail_slug")).after(getParameterValueDropdown());return wrappedInput}function getDetailSlugInputTooltipText(){var text=__("Example: If parameter name is 'contact', the url would be like %1$s/selected-page?contact=2. If this entry is linked to a post, the post permalink will be used instead.","formidable-views");return text.replace("%1$s",frmViewsEditorInfo.siteUrl)}
function getParameterValueDropdown(){var dropdown=select([option("id","ID","id"===dataProcessor.data.parameterValue),option("display_key","Key","display_key"===dataProcessor.data.parameterValue)]);dropdown.id="frm_views_parameter_value";dropdown.classList.add("frm_half");jQuery(document).off("change","#"+dropdown.id);jQuery(document).on("change","#"+dropdown.id,function(){updateDataProcessor("parameterValue",this.value);refreshLayoutBuilderArea()});return dropdown}function refreshDetailSlug(){var detailSlugWrapper=
document.getElementById("frm_views_detail_slug_wrapper");if(null!==detailSlugWrapper)redraw(detailSlugWrapper,getDetailSlugPlaceholder())}function getDisplaySettings(){var children=[];if(!isSingleEntryType()&&!isCalendarType()){children.push(div({class:"frm_grid_container",margin:"0 0 10px",children:[getLimitPlaceholder(),getPageSizePlaceholder()]}));children.push(getAjaxPaginationSettings())}children.push(getDetailSlugPlaceholder(),dataProcessor.placeholder("previewFilter",getPreviewFilterDropdown),
dataProcessor.placeholder("disablePreview",getDisablePreviewCheckbox));if(frmViewsEditorInfo.includeCopyOption)children.push(dataProcessor.placeholder("copy",getCopyViewCheckbox));children.push(getViewKeyInput(),getFormSelectWrapper());return collapsible(div({text:__("Advanced Settings","formidable-views")}),div({children:children}),{open:!isCalendarType()&&!isGridType()&&!isTableType()})}function getAjaxPaginationSettings(){return div({id:"frm_views_ajax_pagination_settings",class:"frm_grid_container",
margin:"0 0 10px",child:getAjaxPaginationPlaceholder()})}function getAjaxPaginationPlaceholder(){var placeholder=dataProcessor.placeholder("ajaxPagination",getAjaxPaginationDropdown);return placeholder}function getAjaxPaginationDropdown(ajaxPagination){return div({children:[tag("label",{text:__("Pagination Type","formidable-views")}),select([option("0",__("Reload full page","formidable-views"),"0"===ajaxPagination),option("1",__("AJAX pagination","formidable-views"),"1"===ajaxPagination),option("load-more",
__("Load more entries","formidable-views"),"load-more"===ajaxPagination),option("infinite-scroll",__("Infinite scroll","formidable-views"),"infinite-scroll"===ajaxPagination)],{id:"frm_views_ajax_pagination",onchange:function(){updateDataProcessor("ajaxPagination",this.value)}})]})}function maybeTogglePaginationSettings(pageSize){var settings=document.getElementById("frm_views_ajax_pagination_settings");if(!settings)return;settings.style.display=""===pageSize?"none":"grid"}function getFormSelectWrapper(){return div({margin:"10px 0 0",
children:[getFormSelectLabel(),getFormSelect()]})}function getLimitPlaceholder(){var limitPlaceholder=dataProcessor.placeholder("limit",getViewLimitInput);limitPlaceholder.classList.add("frm_half");return limitPlaceholder}function getTooltipTrigger(text,placement){var output=span({class:["frm_help","frm_icon_font","frm_tooltip_icon"]});output.setAttribute("title",text);output.style.position="relative";output.style.top="3px";output.style.left="4px";if("undefined"!==typeof placement)output.setAttribute("data-placement",
placement);return output}function getPageSizePlaceholder(){var pageSizePlaceholder=dataProcessor.placeholder("pageSize",getPageSizeInput);pageSizePlaceholder.classList.add("frm_half");return pageSizePlaceholder}function getCalendarSettings(){return collapsible(div({text:__("Calendar Settings","formidable-views")}),dataProcessor.placeholder("fieldOptions",function(){return div({children:[div({children:[tag("label",{text:__("Event Date","formidable-views")}),select(getEventDateOptions(dataProcessor.data.dateFieldId),
{id:"frm_date_field_id",onchange:function(){updateDataProcessor("dateFieldId",this.value)}})]}),div({margin:"10px 0 0",children:[tag("label",{text:__("End Date or Day Count","formidable-views")}),select(getEndDateOrDayCountOptions(dataProcessor.data.edateFieldId),{id:"frm_edate_field_id",onchange:function(){updateDataProcessor("edateFieldId",this.value)}})]}),div({class:"frm_grid_container",children:[getRepeatField(),div({margin:"10px 0 0",class:"frm_half",children:[getEndRepeatLabel(),getEndRepeatSelect()]})]})]})}),
{open:true})}function getRepeatField(){var output=div({margin:"10px 0 0",class:"frm_half",children:[tag("label",{text:__("Repeat","formidable-views")}),select(getEventRepeatOptions(dataProcessor.data.repeatEventFieldId),{id:"frm_repeat_event_field_id",onchange:function(){updateDataProcessor("repeatEventFieldId",this.value)}})]});appendTooltipTriggerAferInputLabel(output,getRepeatFieldTooltipText());return output}function getRepeatFieldTooltipText(){return __("Select a form field that contains values like 1 week, 2 weeks, 1 year, etc. This will set the repeat period for each event.",
"formidable-views")}function getEndRepeatLabel(){var output=tag("label",{text:__("End Repeat","formidable-views")});if(""===dataProcessor.data.repeatEventFieldId)output.classList.add("frm-disabled-label");return output}function getEndRepeatSelect(){var output=select(getEventEndRepeatOptions(dataProcessor.data.repeatEdateFieldId),{id:"frm_repeat_edate_field_id",onchange:function(){updateDataProcessor("repeatEdateFieldId",this.value)}});if(""===dataProcessor.data.repeatEventFieldId)output.disabled=
true;jQuery(document).on("change","#frm_repeat_event_field_id",function(){output.disabled=""===this.value;jQuery(output).prev("label").toggleClass("frm-disabled-label",output.disabled)});return output}function getEventDateOptions(selectedValue){return getFieldOptions(["created_at","updated_at","date"],selectedValue)}function getEndDateOrDayCountOptions(selectedValue){return getFieldOptions(["created_at","updated_at","date","number","select","radio","scale","star"],selectedValue)}function getEventRepeatOptions(selectedValue){var options=
getFieldOptions(["select","radio"],selectedValue);return[option("",__("No repeating events","formidable-views"),""===selectedValue)].concat(options)}function getEventEndRepeatOptions(selectedValue){var options=getFieldOptions(["date"],selectedValue);return[option("",__("Never","formidable-views"),""===selectedValue)].concat(options)}function getSettingsForActiveViewType(){if(isGridType())return getGridStyleSettings();if(isCalendarType())return getCalendarSettings();if(isTableType())return getTableSettings();
return emptyNode()}function getGridStyleSettings(){return collapsible(div({text:__("Grid Style Settings","formidable-views")}),dataProcessor.placeholder("gridColumnCount",function(){setTimeout(initializeColorPicker,0);return div({children:[getCellTypographySettings(),getCellBackgroundSettings(),getCellBorderSettings(),getCellBorderRadiusSettings(),getCellPaddingSettings(),getCellStyleSettings(__("Number of grid columns","formidable-views"),div({child:getNumberOfGridColumnsInput()})),getCellStyleSettings(__("Row gap",
"formidable-views"),div({child:getGridRowGapInput()})),getCellStyleSettings(__("Column gap","formidable-views"),div({child:getGridColumnGapInput()})),getGridResponsivenessWrapper(),getCustomClassesSettings()]})}),{open:true})}function getCustomClassesSettings(){return getCellStyleSettings(tag("label",{text:__("Custom classes","formidable-views"),for:"frm_custom_classes_input"}),div({class:"frm-full-width-settings-input",child:getCustomClassesInput()}))}function getTableSettings(){return collapsible(div({text:__("Table Settings",
"formidable-views")}),div({children:[tag("label",{text:__("Row Style","formidable-views")}),dataProcessor.placeholder("tableRowStyle",getTableRowStyleDropdown),getTableResponsivenessWrapper(),dataProcessor.placeholder("customClasses",getCustomClassesSettings)]}),{open:true})}function getTableRowStyleDropdown(tableRowStyle){return select([option("",__("Match theme settings","formidable-views"),""===tableRowStyle),option("frm-alt-table",__("Zebra stripes","formidable-views"),"frm-alt-table"===tableRowStyle),
option("frm-grid",__("Grid","formidable-views"),"frm-grid"===tableRowStyle)],{id:"frm_table_row_style",onchange:function(){var table=document.getElementById("frm_table_editor_table");dataProcessor.data.tableRowStyle=this.value;jQuery(table).removeClass("frm-alt-table frm-grid");if(""!==this.value)table.classList.add(this.value)}})}function getTableResponsivenessWrapper(){return dataProcessor.placeholder("tableResponsive",function(){var checkbox,label,wrapper;checkbox=getTableResponsivenessInput();
label=tag("label",{text:__("Make table responsive","formidable-views"),fontSize:"14px",for:checkbox.id});wrapper=div({margin:"5px 0 0",children:[checkbox,label]});appendTooltipTriggerAferInputLabel(wrapper,__("If this is on, the each column will display as a full row when the browser window is smaller than 760px width.","formidable-views"),"right");return wrapper})}function getTableResponsivenessInput(){var output=input({id:"frm_table_responsive_"+autoId(),onchange:handleTableResponsivenessChange,
type:"checkbox"});output.checked=!!dataProcessor.data.tableResponsive;return output}function handleTableResponsivenessChange(){dataProcessor.data.tableResponsive=this.checked?1:0;preventUnsavedChanges()}function getGridResponsivenessWrapper(){var checkbox,label,wrapper;checkbox=getGridResponsivenessInput();label=tag("label",{text:__("Make grid responsive","formidable-views"),fontSize:"14px",for:checkbox.id});wrapper=div({children:[checkbox,label]});appendTooltipTriggerAferInputLabel(wrapper,__("If this is on, the number of grid columns will go to one when the browser window is smaller than 750px width.",
"formidable-views"),"right");return wrapper}function getFieldOptions(types,selectedValue){var results=[];if(-1!==types.indexOf("created_at"))results.push(option("created_at",__("Entry creation date","formidable-views"),"created_at"===selectedValue));if(-1!==types.indexOf("updated_at"))results.push(option("updated_at",__("Entry update date","formidable-views"),"updated_at"===selectedValue));each(dataProcessor.data.fieldOptions,function(fieldOption){each(types,function(type){if(type===fieldOption.type)results.push(option(fieldOption.value,
fieldOption.label,selectedValue===fieldOption.value))})});return results}function getMetaboxes(){var wrapper=document.getElementById("frm_view_editor_metabox_wrapper");if(null===wrapper)return emptyNode();return collapsible(div({text:__("Metaboxes","formidable-views")}),wrapper)}function getStyleEditorLeftPanelChildren(){return[getCellSettingsHeader(),separator(),getCellTypographySettings(),getCellBackgroundSettings(),getCellBorderSettings(),getCellBorderRadiusSettings(),getCellPaddingSettings()]}
function getCellSettingsHeader(){var settings=getCellStyleSettings(__("Cell Settings","formidable-views"),div({child:getCellSettingsDoneButton()}));settings.firstChild.style.fontWeight="600";return settings}function getCellSettingsDoneButton(){return button({id:"frm_done_cell_style_settings",onclickPreventDefault:function(){toggleOffCellStyleSettings();saveViewEditor()},text:__("Done","formidable-views")})}function toggleOffCellStyleSettings(event){if(0===state.styleEditorIsActive)return;if("undefined"!==
typeof event&&clickedToTurnOnCellStyleSettings(event))return;state.styleEditorIsActive=0;removeActiveStyleEditorClass();redrawViewEditorLeftPanel();document.body.classList.remove("frm-style-editor-active")}function clickedToTurnOnCellStyleSettings(event){var trigger=event.originalEvent.target;return"svg"===trigger.nodeName&&-1!==trigger.parentNode.id.indexOf("frm_enable_style_editor_")}function dismissEducation(event){event.preventDefault();jQuery(this).closest(".frm-education").parent().remove()}
function dismissComingSoon(){var data={action:"frm_dismiss_coming_soon_message",type:this.getAttribute("data-type"),nonce:frmGlobal.nonce};jQuery(this).closest(".frm-coming-soon").parent().remove();post(data)}function handleSwitchToClassicViewClick(event){event.preventDefault();toggleOffCellStyleSettings();maybeCreateModal({id:"frm_switch_to_classic_view_modal",title:__("Switch to Classic View","formidable-views"),content:getSwitchToClassicViewModalContent(),footer:getSwitchToClassicViewFooter()})}
function hideContentModalOnMediaButtonClick(){var uploader=document.querySelector("#wpwrap ~ div .media-modal");if(null!==uploader){uploader=uploader.parentNode;setupTemporaryBoxContentModalToggle(function(){return"none"===uploader.style.display})}}function hideContentModalOnShortcodesUltimateClick(){var uploader=document.querySelector(".mfp-wrap");if(null!==uploader)setupTemporaryBoxContentModalToggle(function(){return!uploader.classList.contains("mfp-ready")})}function setupTemporaryBoxContentModalToggle(statusCallback){var interval;
temporarilyHideBoxContentModal();interval=setInterval(function(){if(statusCallback()){setBoxContentModalVisibilityBack();clearInterval(interval)}},50)}function getSwitchToClassicViewModalContent(){var line2Text=__("This is irreversible. You will not be able to change back to a %1$s after updating.","formidable-views");line2Text=line2Text.replace("%1$s",isGridType()?getGridTypeDescriptionText():getTableTypeDescriptionText());return div({padding:"20px",children:[div({text:__("Switching to classic gives you full control of the HTML, but it means that you will no longer have access to the layout builder or style editor tools.",
...
children:[checkbox,label]})}function getDisablePreviewCheckbox(disablePreview){var id="frm_views_disable_preview";var labelText=__("Show code for this view","formidable-views");var checked=disablePreview;var $jscomp$destructuring$var0=getSettingsCheckboxElements(id,labelText,checked);var checkbox=$jscomp$destructuring$var0.checkbox;var label=$jscomp$destructuring$var0.label;if(disablePreview)toggleCodeViewActiveClass(true);checkbox.disabled=isLegacyTableType();checkbox.addEventListener("change",function(){updateDataProcessor("disablePreview",
checkbox.checked?1:0);toggleCodeViewActiveClass(checkbox.checked);refreshActivePreviews()});return div({margin:"10px 0 0",children:[checkbox,label,getTooltipTrigger(getShowCodeForThisViewTooltipText())]})}function getSettingsCheckboxElements(id,labelText,checked){var checkboxArgs={id:id};var checkbox=tag("input",checkboxArgs);checkbox.setAttribute("type","checkbox");var label=tag("label",{text:labelText,for:checkbox.id});if(checked)checkbox.checked=true;return{checkbox:checkbox,label:label}}function toggleCodeViewActiveClass(on){jQuery(document.getElementById("frm_view_editor_preview_area")).toggleClass("frm-code-view-active",
on)}function getShowCodeForThisViewTooltipText(){return __("This box is forcibly checked if you include a <table> tag in the Before Content box. If you are editing a view and notice the visual tab is selected and the table HTML is missing, you can switch to the HTML tab, close the content pop up, and reopen it. As long as the settings have not been saved, the old HTML will be back to way it was before loading it in the visual tab. While this is checked, the preview will also display the HTML as code.",
...
margin:"10px 0 0 0"})}function getShortcodeExample(){return div({id:"frm_views_shortcode_example",child:getExampleWithCopyAction("[display-frm-data id="+frmViewsEditorViewData.id+"]",{margin:"15px 0 0"})})}function getApplicationDropdown(){if("undefined"===typeof frmDom)return emptyNode();var container=div();post({action:"frm_get_applications_data",view:"applications",sort:"alphabet",nonce:frmGlobal.nonce},function(data){if(!data.applications.length)return;var options=[option("","",0===frmViewsEditorViewData.applicationIds.length)];
data.applications.forEach(function(applicationData){return options.push(option(applicationData.termId,applicationData.name,-1!==frmViewsEditorViewData.applicationIds.indexOf(applicationData.termId)))});var label=tag("label",{text:__("Insert view into application","formidable-views")});var onchange=function(){frmViewsEditorViewData.applicationIds=jQuery(dropdown).val()};var dropdownArgs={id:"frm_view_application_ids",onchange:onchange};var dropdown=select([],dropdownArgs);dropdown.classList.add("frm_multiselect");
dropdown.setAttribute("multiple","multiple");options.forEach(function(option){return dropdown.appendChild(option)});container.appendChild(label);container.appendChild(dropdown);container.className="frm-fields";container.style.margin="15px 0 10px";var $dropdown=jQuery(dropdown);$dropdown.hide().each(frmDom.bootstrap.multiselect.init);$dropdown.on("frm-multiselect-changed",adustValuesForApplicationDropdown)});return container}function adustValuesForApplicationDropdown(_,option){if(""!==option.getAttribute("value"))unselectBlankOptionIfSelected(jQuery(this))}
function unselectBlankOptionIfSelected($select){var selectedValues=$select.val(),index;if(selectedValues===null){$select.next(".btn-group").find('.multiselect-container input[value=""]').prop("checked",true);onEveryoneOptionSelected($select);return}index=selectedValues.indexOf("");if(index>=0){selectedValues.splice(index,1);$select.val(selectedValues);$select.next(".btn-group").find('.multiselect-container input[value=""]').prop("checked",false)}}function getPhpEchoExample(){return getExampleWithCopyAction("<?php echo FrmViewsDisplaysController::get_shortcode( array( 'id' => "+
frmViewsEditorViewData.id+" ) ); ?>",{margin:"10px 0 15px"})}function getExampleWithCopyAction(text,wrapperArgs){if("undefined"===typeof frmViewsEditorViewData){console.warn("formidable-views: frmViewsEditorViewData is not set, unable to get example with copy action");return div()}if("undefined"===typeof wrapperArgs)wrapperArgs={};wrapperArgs.children=[div({text:text}),getCopyAction(text)];wrapperArgs.class="frm-example-with-copy-action";return div(wrapperArgs)}function getCopyAction(text){return a({text:__("Copy",
"formidable-views"),id:"frm_copy_"+getUniqueCopyId(),onclickPreventDefault:onCopyActionClick(text)})}function getUniqueCopyId(){return copyAutoId++}function onCopyActionClick(text){return function(){copyToClipboard(text)}}function getNoEntriesTextarea(){var output=tag("textarea");output.id="frm_view_editor_empty_message";dataProcessor.get("emptyMessage",function(emptyMessage){output.value=emptyMessage});return output}function getViewEditorRightPanel(){if(isSingleEntryType())return getViewEditorRightPanelForSingleEntryType();
return getViewEditorRightPanelForDynamicType()}function getViewEditorRightPanelForSingleEntryType(){return div({id:"frm_view_editor_right",child:dataProcessor.placeholder("listingContent",getListingPage)})}function getViewEditorRightPanelForDynamicType(){var pageWrapper=div();return div({id:"frm_view_editor_right",children:[getListingDetailsPageToggles(pageWrapper),separator(),pageWrapper]})}function getListingDetailsPageToggles(pageWrapper){return div({class:"frmcenter",child:tabs([{text:__("Listing Page",
"formidable-views"),id:"frm_views_trigger_listing_page",page:getListingPage,pageKey:"listing"},{text:__("Detail Page","formidable-views"),id:"frm_views_trigger_detail_page",page:getDetailPage,pageKey:"detail"}],{active:state.activeTab,tabClass:"frm-listing-details-page-toggle",wrapperClass:"frm-listing-details-page-toggles",pageWrapper:pageWrapper,stateHandler:onPageToggleUpdate})})}function getListingPage(){var addBeforeContentWrapper=getBeforeContentWrapper(),addAfterContentWrapper=getAfterContentWrapper();
return getViewEditorPreviewArea([getBeforeLayoutBuilderToggleOptions(),comingSoonPlaceholder(),addBeforeContentWrapper,getLayoutBuilderArea(),addAfterContentWrapper])}function getDetailPage(){return getViewEditorPreviewArea([getBeforeLayoutBuilderToggleOptions(),getLayoutBuilderArea()])}function getViewEditorPreviewArea(children){var output=div({id:"frm_view_editor_preview_area",class:useTableTags()?"frm_table_preview_area":"frm_div_preview_area",padding:"0 60px 30px",children:children});if(dataProcessor.data.disablePreview)output.classList.add("frm-code-view-active");
return output}function getBeforeContentWrapper(){return getBeforeOrAfterContentWrapper("before")}function getBeforeOrAfterContentWrapper(type){var key="before"===type?"listingBeforeContent":"listingAfterContent";return dataProcessor.placeholder(key,function(html){var args={},children=[],wrapperArgs,wrapper;if("before"===type)updateBeforeContentInput(html);if(html.length){wrapperArgs={class:"frm-disable-clicking-in-content"};if(useHtmlInsteadOfPreview())wrapperArgs.text=html;else wrapperArgs.html=
html;wrapper=div(wrapperArgs);boxProcessor.watch(type+"Content",function(){var processedHtml=boxProcessor.state.details[type+"Content"];processedHtml=processedHtml.replace(/\[siteurl\]/g,frmViewsEditorInfo.siteUrl);processedHtml=processedHtml.replace(/\[sitename\]/g,frmViewsEditorInfo.siteName);processedHtml=processedHtml.replace(/\[entry_count\]/g,boxProcessor.state.details.entryCount);if(useHtmlInsteadOfPreview())wrapper.textContent=processedHtml;else wrapper.innerHTML=processedHtml});children.push(wrapper)}else children.push(div({class:"frm-add-"+
type+"-icon",icon:"frm_views_editor_plus_circle"}),div({class:"frm-add-"+type+"-content",text:getBeforeOrAfterContentWrapperText(type)}));args.child=div({id:"frm_"+type+"_view_content_wrapper",onclick:function(){var boxData={id:0,useContent:type+"_content"};editBoxContentModal(boxData)},class:["frm-working-space-border","with_frm_style","frm_clearfix"],children:children});if(!html.length)args.child.classList.add("frm-empty-content-wrapper");return div(args)})}function getBeforeOrAfterContentWrapperText(type){switch(type){case "after":return __("Add After Content",
...
andOrSelect=getAndOrSelect(filter.or,1!==index);valueInput=getFilterValueInput(filter.where,filter.val);fieldSelect=getFieldSelect(filter.where,valueInput);if(1===index)andOrSelect.classList.add("frm-target-and-or-select");return div({margin:"10px 0",children:[andOrSelect,fieldSelect,getOperatorSelect(filter.is),valueInput]})}function getFilterValueInput(where,value){var statusDropdown,draftDropdown,valueInput;if("is_draft"===where){draftDropdown=select(getDraftDropdownOptions(value));draftDropdown.classList.add("frm-filter-value");
return draftDropdown}if(frmViewsEditorInfo.statusFieldId===parseInt(where)){statusDropdown=select(getStatusDropdownOptions(value));statusDropdown.classList.add("frm-filter-value");return statusDropdown}valueInput=input({value:value,placeholder:__("Enter value","formidable-views"),class:"frm-filter-value"});if(fieldIsADateField(where))setTimeout(function(){setupDateFilter(valueInput)},0);return valueInput}function getDraftDropdownOptions(selectedValue){return Object.keys(frmViewsEditorInfo.draftDropdownOptions).map(function(value){return option(value,
frmViewsEditorInfo.draftDropdownOptions[value],selectedValue===value)})}function getStatusDropdownOptions(selectedValue){return Object.keys(frmViewsEditorInfo.statusDropdownOptions).map(function(value){return option(value,frmViewsEditorInfo.statusDropdownOptions[value],selectedValue===value)})}function getAndOrSelect(orSelected,disabled){var output=select([option("and",__("And","formidable-views"),!orSelected),option("or",__("Or","formidable-views"),orSelected)]);output.classList.add("frm-and-or-select");
if(true===disabled)output.disabled=true;return output}function getFieldSelect(selectedValue,valueInput){var isSort,selectArgs,output,previousValue;if("undefined"===typeof selectedValue)selectedValue="";isSort="undefined"===typeof valueInput;selectArgs={};previousValue=selectedValue;selectArgs.id="frm_field_select_"+autoId();if(isSort)selectArgs.onchange=function(){if("rand"===this.value||"rand"===previousValue)jQuery(this).toggleClass("frm-random-order-option","rand"===this.value);previousValue=this.value};
else selectArgs.onchange=function(){var newValueInput;if("is_draft"===this.value||"is_draft"===previousValue){newValueInput=getFilterValueInput(this.value,valueInput.value);jQuery(valueInput).replaceWith(newValueInput);valueInput=newValueInput}if(frmViewsEditorInfo.statusFieldId===parseInt(this.value)||frmViewsEditorInfo.statusFieldId===parseInt(previousValue)){newValueInput=getFilterValueInput(this.value,valueInput.value);jQuery(valueInput).replaceWith(newValueInput);valueInput=newValueInput}if(fieldIsADateField(this.value))setupDateFilter(valueInput);
...
function triggerLayoutBuilderModal(){var modalId="frm_layout_builder_modal";state.editingLayoutTemplate=false;state.layoutBuilderModalData=deepClone(getActiveLayoutData());resetBoxAutoId();maybeCreateModal({id:modalId,title:__("Build a layout","formidable-views"),content:getLayoutBuilderModalContent(),footer:getLayoutBuilderModalFooter(modalId)});makeLayoutTypeOptionsDraggable();makeWorkingAreaDroppable();if(!layoutDataIsAlreadySet())addLayoutBuilderModalEducation()}function resetBoxAutoId(){boxAutoId=
