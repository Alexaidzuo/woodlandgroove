...
tabs;frmViewsDom.collapsible=collapsible;frmViewsDom.redraw=redraw;frmViewsDom.success=success;frmViewsModal.maybeCreateModal=maybeCreateModal;frmViewsModal.recenterModal=recenterModal;frmViewsUtil.each=each;frmViewsUtil.map=map;frmViewsUtil.makeHas=makeHas;frmViewsUtil.post=post;frmViewsTable.getTableSelectAllAndSearchOptions=getTableSelectAllAndSearchOptions})();(function(){if("undefined"===typeof wp)return;var tag,div,span,a,input,emptyNode,button,select,option,pullOptionsFrom,maybeCreateModal,post,each,map,getTableSelectAllAndSearchOptions;var registeredModalListeners=false;var currentAutoId=0;var __=wp.i18n.__;var state={viewName:"",applicationId:0};var canAddApplications="1"===frmViewsIndexVars.canAddApplications;function triggerSelectViewType(){return maybeCreateModal({id:"frm_new_view_modal",title:__("Select View type","formidable-views"),content:selectViewTypeContent(),
footer:selectViewTypeFooter()})}function triggerStepTwo(type){state.viewName="undefined"!==typeof frmSelectedFormName?frmSelectedFormName+" "+getTypeSuffix(type):"";maybeCreateModal({id:"frm_new_view_modal",title:getStepTwoTitle(),content:selectDataSourceContent(type),footer:selectDataSourceFooter(type)})}function getStepTwoTitle(){return getFormIdFromUrl()?__("Create new view","formidable-views"):__("Select Data Source","formidable-views")}function triggerCreateView(formId,type){var data={action:"frm_views_editor_create",
form:formId,type:type,nonce:frmGlobal.nonce,name:getViewName(),application_id:state.applicationId};if("table"===type)data.tableOptions=getSelectedTableOptionFieldValues();post(data)}function getViewName(){return state.viewName}function getSelectedTableOptionFieldValues(){var fieldValues=[];each(document.querySelectorAll(".frm-new-table-view-option input"),function(input){if(input.checked)fieldValues.push(input.value)});return fieldValues}function getFormIdValueFromDatasourceSelect(){return document.getElementById("frm_new_view_datasource_select").value}
function onClickPreventDefault(id,callback){jQuery("#"+id).click(function(event){event.preventDefault();callback()})}function onNewViewClick(){triggerSelectViewType();if(registeredModalListeners)return;registeredModalListeners=true;jQuery(document).on("click","#frm_new_view_modal li .frm-hover-icons .frm-create-view",function(event){var type=jQuery(this).closest("[frm-view-type]").attr("frm-view-type");if("application"===type)return;event.preventDefault();triggerStepTwo(type)});jQuery(document).on("click",
'#frm_new_view_modal li [role="button"]:not(a)',handleNewViewTypeContainerClick)}function handleNewViewTypeContainerClick(){var li=this.closest("li");var hoverIcons=li.querySelector(".frm-hover-icons");if(!hoverIcons){handleSelectableMouseOver.bind(li)();hoverIcons=li.querySelector(".frm-hover-icons");hoverIcons.style.display="none"}var trigger=hoverIcons.querySelector(".frm-create-view");trigger.click()}function selectViewTypeContent(){var list=tag("ul",{class:["frm-templates-list","frm-categories-list"],
children:[getGridOption(),getCalendarViewOption(),getTableOption(),getClassicViewOption()]});if("string"===typeof frmGlobal.applicationsUrl&&frmGlobal.canAccessApplicationDashboard)list.appendChild(getReadyMadeSolutionOption());var wrapper=div({class:["cta-inside","frmcenter"],child:list});return div({class:"inside",child:wrapper})}function getGridOption(){return viewTypeListItem("grid","frm_view_icon_grid","#805EF6",__("Grid","formidable-views"),__("Create a view and write less code","formidable-views"))}
...
"formidable-views"),__("Create a new view from scratch","formidable-views"))}function getReadyMadeSolutionOption(){var item=viewTypeListItem("application","frm_views_editor_ready_made_solution_icon","var(--primary-color)",__("Ready Made Solutions","formidable-views"),__("Start from an application template","formidable-views"));handleSelectableMouseOver.bind(item)();handleSelectableMouseOut.bind(item)();var newViewTrigger=item.querySelector(".frm-create-view");newViewTrigger.setAttribute("href",frmGlobal.applicationsUrl);
return item}function selectViewTypeFooter(){return div({class:"frmcenter",child:document.createTextNode(getCurrentStepDescription(1))})}function getCurrentStepDescription(currentStep,type){var numberOfSteps=getTotalNumberOfSteps(type);if(1===numberOfSteps)return"";return __("%1$s of %2$s","formidable_views").replace("%1$s",currentStep).replace("%2$s",numberOfSteps)}function typeIncludesThreeSteps(type){return"table"===type}function getTotalNumberOfSteps(type){var numberOfSteps=typeIncludesThreeSteps(type)?
3:2;return numberOfSteps}function getFormIdFromUrl(){return getUrlParam("form")}function getUrlParam(key){var queryString=window.location.search;var urlParams=new URLSearchParams(queryString);return urlParams.get(key)}function selectDataSourceContent(type){var selectedDataSourceContent=div({class:["frm_wrap","field-group"],children:[maybeGetComingSoon(type),getViewNameInputWrapper(),getFormSelectLabel(),getFormSelect(type)],padding:"30px"});var applicationDropdown=maybeGetApplicationDropdown();if(applicationDropdown)selectedDataSourceContent.appendChild(applicationDropdown);
return selectedDataSourceContent}function getFormSelectLabel(){if(getFormIdFromUrl())return emptyNode();var label=tag("label",{text:__("Use Entries from Form","formidable-views"),fontSize:"14px"});label.setAttribute("for","frm_new_view_datasource_select");return div({child:label,margin:"15px 0 8px 0"})}function getViewNameInputWrapper(){var inputId="frm_new_view_name";var nameLabel=tag("label",{text:__("View Name","formidable-views"),fontSize:"14px"});nameLabel.setAttribute("for",inputId);return div({children:[div({margin:"0 0 8px 0",
child:nameLabel}),div({child:input({id:inputId,value:state.viewName,onchange:function(){state.viewName=this.value},fontSize:"14px"})})]})}function maybeGetApplicationDropdown(){if("undefined"===typeof frmDom)return false;if(!canAddApplications)return false;var dropdown=getApplicationAutocomplete();var label=tag("label",{fontSize:"14px",text:__("Insert view into application","formidable-views")});label.setAttribute("for","frm_application_autocomplete_input");label.appendChild(span({class:"frm-new-pill",
text:__("NEW","formidable")}));var wrapper=div({margin:"15px 0 0 0",children:[div({child:label,margin:"15px 0px 8px"}),dropdown]});frmDom.autocomplete.initAutocomplete("application",dropdown);setTimeout(function(){var modal=document.getElementById("frm_new_view_modal");if(modal){modal.classList.add("frm_wrap");jQuery(dropdown.querySelector(".frm-application-search")).autocomplete("option","appendTo",modal)}},0);var hiddenInput=dropdown.querySelector(".frm_autocomplete_value_input");state.applicationId=
getUrlParam("applicationId");if(state.applicationId&&"undefined"!==typeof frmAutocompleteApplicationName){dropdown.querySelector("#frm_application_autocomplete_input").value=frmAutocompleteApplicationName;hiddenInput.value=state.applicationId}jQuery(dropdown).on("autocompletechange",function(){return state.applicationId=hiddenInput.value});return wrapper}function getApplicationAutocomplete(){return span({children:[input({id:"frm_application_autocomplete_input",class:"frm-application-search",type:"text",
placeholder:__("Select application","formidable-pro")}),input({class:"frm_autocomplete_value_input",type:"hidden",name:"application_id"})]})}function maybeGetComingSoon(type){if(-1===["calendar"].indexOf(type))return emptyNode();return comingSoon(type)}function comingSoon(type){var icon,title;title=__("The new %1$s is coming soon.","formidable-views");switch(type){case "calendar":icon="frm_views_editor_coming_soon_calendar";title=title.replace("%1$s",__("Calendar Editor","formidable-views"));break}return div({class:["frm-coming-soon",
"frm_grid_container"],children:[div({class:"frm2",icon:icon}),div({class:"frm10",children:[span({class:"frm-coming-soon-pill",text:__("COMING SOON","formidable-views")}),div({margin:"5px 0 0",children:[span({text:title}),document.createTextNode(" "),getFormidableSubscribeLink(),document.createTextNode(" "),span({text:__("to receive news about the latest updates.","formidable-views")})]})]})]})}function getFormidableSubscribeLink(){return a({text:__("Subscribe to Formidable Forms","formidable-views"),
href:"https://formidableforms.com/newsletter/",target:"_blank"})}function getFormSelect(type){if(getFormIdFromUrl())return emptyNode();return select([option("",__("Select datasource","formidable-views"))].concat(pullOptionsFrom("frm_data_source_form_select_template")),{id:"frm_new_view_datasource_select",onchange:function(){var formName=this.querySelector('option[value="'+this.value+'"]').textContent.trim();state.viewName=formName+" "+getTypeSuffix(type);document.getElementById("frm_new_view_name").value=
state.viewName}})}function getTypeSuffix(type){if("all"===type||type.length<2)return"View";return type[0].toUpperCase()+type.substr(1)}function selectDataSourceFooter(type){var backButton=button({text:__("Back","formidable-views"),id:"frm_select_data_source_back",onclickPreventDefault:triggerSelectViewType});var pageCounter=span({class:"frm-modal-pagecounter",child:document.createTextNode(getCurrentStepDescription(2,type))});var createButton=button({text:__("Create a view","formidable-views"),id:"frm_select_data_source_create",
onclickPreventDefault:function(){var formId;formId=getFormIdFromUrl();if(!formId)formId=getFormIdValueFromDatasourceSelect();if(typeIncludesThreeSteps(type)&&""!==formId)transitionToStepThree(formId,type);else triggerCreateView(formId,type)}});return div({children:[pageCounter,backButton,createButton],position:"relative"})}function transitionToStepThree(formId,type){return maybeCreateModal({id:"frm_new_view_modal",title:__("Customize view","formidable-views"),content:stepThreeContent(formId,type),
footer:stepThreeFooter(formId,type)})}function stepThreeContent(formId,type){if("table"===type)return getStepThreeContentForTableType(formId);return emptyNode()}function getStepThreeContentForTableType(formId){var placeholder=div();post({action:"frm_views_get_table_column_options",form:formId,nonce:frmGlobal.nonce},function(data){placeholder.appendChild(getTableOptionHtmlFromData(data))});return div({padding:"20px",children:[getTableSelectAllAndSearchOptions(),div({margin:"0 0 15px",text:__("Select columns that should be displayed in view",
"formidable-views")}),placeholder]})}function getTableOptionHtmlFromData(data){return div({class:["frm_grid_container","frm-table-options-container"],children:map(data.options,function(optionInfo){var checkboxId="frm_table_option_checkbox_"+autoId();var label=tag("label",{text:optionInfo.label});label.setAttribute("for",checkboxId);return div({class:["frm_half","frm-new-table-view-option"],children:[getTableOptionCheckbox(optionInfo.value,checkboxId),label]})})})}function autoId(){return++currentAutoId}
function getTableOptionCheckbox(value,checkboxId){var checkbox=tag("input");checkbox.id=checkboxId;checkbox.setAttribute("type","checkbox");checkbox.value=value;return checkbox}function stepThreeFooter(formId,type){var backButton=button({text:__("Back","formidable-views"),id:"frm_select_data_source_back",onclickPreventDefault:function(){triggerStepTwo(type)}});var pageCounter=span({class:"frm-modal-pagecounter",child:document.createTextNode(getCurrentStepDescription(3,type))});var createButton=button({text:__("Create a view",
"formidable-views"),id:"frm_select_data_source_create",onclickPreventDefault:function(){triggerCreateView(formId,type)}});return div({children:[pageCounter,backButton,createButton],position:"relative"})}function viewTypeListItem(type,icon,backgroundColor,title,description){var li=tag("li",{class:"frm-selectable"});var wrapper=div({class:"frm-featured-form",children:[div({class:"frm-category-icon",icon:icon,backgroundColor:backgroundColor,role:"button"}),div({children:[tag("h3",{text:title,class:"view-type-title",
role:"button"}),div({text:description,class:"view-type-description",role:"button"})]})]});li.appendChild(wrapper);li.setAttribute("frm-view-type",type);return li}function handleSelectableMouseOver(){var icons=this.querySelector(".frm-hover-icons");if(null!==icons)icons.style.display="inline-block";else{var clone=document.getElementById("frm-hover-icons-template").cloneNode(true);clone.id="";this.appendChild(clone)}}function handleSelectableMouseOut(){var icons=this.querySelector(".frm-hover-icons");
if(null!==icons)icons.style.display="none"}function handleTableOptionSelect(){jQuery(this).closest(".frm-new-table-view-option").toggleClass("frm-selected-table-view-option",this.checked)}function maybeTriggerNewViewModalOnLoad(){if(!getUrlParam("triggerNewViewModal"))return;onNewViewClick()}if("undefined"!==typeof frmViewsDom){tag=frmViewsDom.tag;div=frmViewsDom.div;span=frmViewsDom.span;a=frmViewsDom.a;input=frmViewsDom.input;emptyNode=frmViewsDom.emptyNode;button=frmViewsDom.button;select=frmViewsDom.select;
option=frmViewsDom.option;pullOptionsFrom=frmViewsDom.pullOptionsFrom}if("undefined"!==typeof frmViewsModal)maybeCreateModal=frmViewsModal.maybeCreateModal;if("undefined"!==typeof frmViewsUtil){post=frmViewsUtil.post;each=frmViewsUtil.each;map=frmViewsUtil.map}if("undefined"!==typeof frmViewsTable)getTableSelectAllAndSearchOptions=frmViewsTable.getTableSelectAllAndSearchOptions;maybeTriggerNewViewModalOnLoad();onClickPreventDefault("frm_new_view",onNewViewClick);jQuery(document).on("change",'.frm-new-table-view-option input[type="checkbox"]',
handleTableOptionSelect);jQuery(document).on("mouseover",".frm-modal .frm-selectable",handleSelectableMouseOver);jQuery(document).on("mouseout",".frm-modal .frm-selectable",handleSelectableMouseOut)})();
