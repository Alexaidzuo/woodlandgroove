...
mediaIDs[m],fieldName))});this.on("complete",function(file){var fileName,node,img,thumbnail;processesRunning--;frmFrontForm.removeSubmitLoading(form,"enable",processesRunning);if(typeof file.mediaID!=="undefined"){if(uploadFields[i].uploadMultiple)jQuery(file.previewElement).append(getHiddenUploadHTML(uploadFields[i],file.mediaID,fieldName));fileName=file.previewElement.querySelectorAll("[data-dz-name]");for(var _i=0,_len=fileName.length;_i<_len;_i++){node=fileName[_i];if(file.accessible)node.innerHTML=
...
0;f<uploadFields[i].mockFiles.length;f++){var mockFile={name:uploadFields[i].mockFiles[f].name,size:uploadFields[i].mockFiles[f].size,url:uploadFields[i].mockFiles[f].file_url,mediaID:uploadFields[i].mockFiles[f].id,accessible:uploadFields[i].mockFiles[f].accessible,ext:uploadFields[i].mockFiles[f].ext,type:uploadFields[i].mockFiles[f].type};this.emit("addedfile",mockFile);if(mockFile.accessible&&"string"===typeof mockFile.type&&0===mockFile.type.indexOf("image/"))this.emit("thumbnail",mockFile,mockFile.url);
this.emit("complete",mockFile);this.files.push(mockFile)}}})}function maybeGetExtensionThumbnail(ext){if("pdf"===ext)return getProPluginUrl()+"/images/pdf.svg";else if(-1!==ext.indexOf("doc"))return getProPluginUrl()+"/images/doc.svg";else if(-1!==ext.indexOf("xls"))return getProPluginUrl()+"/images/xls.svg";return false}function getProPluginUrl(){var freePluginUrlSplitBySlashes=frm_js.images_url.split("/");freePluginUrlSplitBySlashes.pop();freePluginUrlSplitBySlashes.pop();freePluginUrlSplitBySlashes.push("formidable-pro");
...
depFieldArgs.fieldKey+'-"]')}function clearValueForInputs(inputs,required){var prevInput,blankSelect,valueChanged,l,i,input;if(inputs.length<1)return;valueChanged=true;l=inputs.length;for(i=0;i<l;i++){input=inputs[i];if(input.className.indexOf("frm_dnc")>-1||input.name.indexOf("[row_ids]")>-1){prevInput=input;continue}if(i>0&&prevInput.name!=input.name&&valueChanged===true)triggerChange(jQuery(prevInput));valueChanged=true;if(input.type==="radio"||input.type==="checkbox")input.checked=false;else if(input.tagName===
"SELECT"){blankSelect=input.selectedIndex===0&&input.options[0].text.trim()==="";if(blankSelect||input.selectedIndex===-1)valueChanged=false;else input.selectedIndex=-1;var chosenId=input.id.replace(/[^\w]/g,"_");var autocomplete=document.getElementById(chosenId+"_chosen");if(autocomplete!==null)jQuery(input).trigger("chosen:updated")}else if(input.type==="range")input.value=0;else if(input.getAttribute("data-frmprice")!==null)setHiddenProduct(input);else{input.value="";if(null!==input.getAttribute("data-frmfile"))clearDropzoneFiles(input)}if(required===
...
function getLookupValues(childFieldArgs,callback){disableFormPreLookup(childFieldArgs.formId);jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_replace_lookup_field_options",parent_fields:childFieldArgs.parents,parent_vals:childFieldArgs.parentVals,field_id:childFieldArgs.fieldId,nonce:frm_js.nonce},success:function(newOptions){enableFormAfterLookup(childFieldArgs.formId);callback(newOptions)}})}function triggerLookupOptionsLoaded($fieldDiv){$fieldDiv.trigger("frmLookupOptionsLoaded")}
...
function showAddButton(sectionID){jQuery("#frm_field_"+sectionID+"_container .frm_add_form_row").removeClass("frm_hide_add_button")}function addRow(){var thisBtn,id,i,numberOfSections,lastRowIndex,stateField,state;if(currentlyAddingRow===true)return false;currentlyAddingRow=true;thisBtn=jQuery(this);id=thisBtn.data("parent");i=0;numberOfSections=jQuery(".frm_repeat_"+id).length;if(numberOfSections>0){lastRowIndex=jQuery(".frm_repeat_"+id).last().attr("id").replace("frm_section_"+id+"-","");if(lastRowIndex.indexOf("i")>
-1)i=1;else i=1+parseInt(lastRowIndex)}stateField=document.querySelector('input[name="frm_state"]');state=null!==stateField?stateField.value:"";jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"json",data:{action:"frm_add_form_row",field_id:id,i:i,numberOfSections:numberOfSections,nonce:frm_js.nonce,frm_state:state},success:function(r){if(r.html){var html=r.html;var item=jQuery(html).addClass("frm-fade-in");thisBtn.parents(".frm_section_heading").append(item);if(r.is_repeat_limit_reached)hideAddButton(id);
var checked=["other"];var fieldID,fieldObject;var reset="reset";var repeatArgs={repeatingSection:id.toString(),repeatRow:i.toString()};jQuery(html).find("input, select, textarea").each(function(){if(this.name==="")return true;if(this.type=="file")fieldID=this.name.replace("file","").split("-")[0];else fieldID=this.name.replace("item_meta[","").split("]")[2].replace("[","");if(jQuery.inArray(fieldID,checked)==-1){if(this.id===false||this.id==="")return;fieldObject=jQuery("#"+this.id);checked.push(fieldID);
...
total=formatCurrency(total,currency);formatted=totalField.prev(".frm_total_formatted");if(formatted.length<1)formatted=totalField.closest(".frm_form_field").find(".frm_total_formatted");if(formatted.length)formatted.html(total)})}function formatCurrency(total,currency){var leftSymbol,rightSymbol;total=maybeAddTrailingZeroToPrice(total,currency);total=maybeRemoveTrailingZerosFromPrice(total,currency);total=addThousands(total,currency);leftSymbol=currency.symbol_left+currency.symbol_padding;rightSymbol=
currency.symbol_padding+currency.symbol_right;return leftSymbol+total+rightSymbol}function maybeRemoveTrailingZerosFromPrice(total,currency){var split=total.split(currency.decimal_separator);if(2!==split.length||split[1].length<=currency.decimals)return total;if(0===currency.decimals)return split[0];return split[0]+currency.decimal_separator+split[1].substr(0,currency.decimals)}function addRteRequiredMessages(){var keys,length,index,key,field;if("undefined"===typeof __FRMRTEREQMESSAGES)return;keys=
Object.keys(__FRMRTEREQMESSAGES);length=keys.length;for(index=0;index<length;++index){key=keys[index];field=document.getElementById(key);if(field)field.setAttribute("data-reqmsg",__FRMRTEREQMESSAGES[key])}}function isProductFieldHidden(input){return input.getAttribute("data-frmhidden")!==null}function isRepeatingWithTotal(input,total){var regex=/item_meta\[.+?\]\[.+?\]/;return isRepeatingFieldByName(input.name)&&total.name.match(regex)[0]===input.name.match(regex)[0]}function getCurrency(formId){if(typeof __FRMCURR!==
...
currency.thousand_separator);return total}return{init:function(){jQuery(document).on("frmFormComplete",afterFormSubmitted);jQuery(document).on("frmPageChanged",afterPageChanged);jQuery(document).on("frmAfterAddRow frmAfterRemoveRow",calcProductsTotal);jQuery(document).on("click",".frm_trigger",toggleSection);var $blankField=jQuery(".frm_blank_field");if($blankField.length)$blankField.closest(".frm_toggle_container").prev(".frm_trigger").trigger("click");jQuery(document).on("click",".frm_remove_link",
removeFile);jQuery(document).on("focusin","input[data-frmmask]",function(){jQuery(this).mask(jQuery(this).data("frmmask").toString(),{autoclear:false})});jQuery(document).on("frmFieldChanged",maybeCheckDependent);jQuery(document).on("keyup","input.frm_strength_meter",checkPasswordField);jQuery(document).on("keydown",".frm_switch",triggerToggleClickOnSpace);jQuery(document).on("mouseenter click",".frm-star-group input",loadStars);jQuery(document).on("mouseenter",".frm-star-group .star-rating:not(.star-rating-readonly)",
hoverStars);jQuery(document).on("mouseleave",".frm-star-group .star-rating:not(.star-rating-readonly)",unhoverStars);jQuery(document).on("click",'.frm-show-form input[type="submit"], .frm-show-form input[name="frm_prev_page"], .frm_page_back, .frm_page_skip, .frm-show-form .frm_save_draft, .frm_prev_page, .frm_button_submit',setNextPage);jQuery(document).on("change",'.frm_other_container input[type="checkbox"], .frm_other_container input[type="radio"], .frm_other_container select',showOtherText);
jQuery(document).on("change",'.frm_switch_block input[type="checkbox"]',setToggleAriaChecked);jQuery(document).on("click",".frm_remove_form_row",removeRow);jQuery(document).on("click",".frm_add_form_row",addRow);jQuery(".frm_edit_link_container").on("click","a.frm_inplace_edit",editEntry);jQuery(".frm_edit_link_container").on("click","a.frm_cancel_edit",cancelEdit);jQuery(document).on("click",".frm_ajax_delete",deleteEntry);jQuery(".frm_month_heading, .frm_year_heading").on("click",function(){var content=
jQuery(this).children(".ui-icon-triangle-1-e, .ui-icon-triangle-1-s");if(content.hasClass("ui-icon-triangle-1-e")){content.addClass("ui-icon-triangle-1-s").removeClass("ui-icon-triangle-1-e");jQuery(this).next(".frm_toggle_container").fadeIn("slow")}else{content.addClass("ui-icon-triangle-1-e").removeClass("ui-icon-triangle-1-s");jQuery(this).next(".frm_toggle_container").hide()}});addTopAddRowBtnForRepeater();jQuery(document).on("click",'input[type="checkbox"][data-frmlimit]',checkCheckboxSelectionLimit);
