...
if ( ! function_exists( 'woodlandgroove_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function woodlandgroove_posted_on() {
		...
		$posted_on = sprintf(
			esc_html_x( 'Posted on %s', 'post date', 'woodlandgroove' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			esc_html_x( 'by %s', 'post author', 'woodlandgroove' ),
...
if ( ! function_exists( 'woodlandgroove_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function woodlandgroove_entry_footer() {
		// Hide category and tag text for pages.
		if ( 'post' == get_post_type() ) {
			/* translators: used between list items, there is a space after the comma */
			$categories_list = get_the_category_list( esc_html__( ', ', 'woodlandgroove' ) );
			if ( $categories_list ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'woodlandgroove' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'woodlandgroove' ) );
			if ( $tags_list ) {
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'woodlandgroove' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}

		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link">';
			comments_popup_link( esc_html__( 'Leave a comment', 'woodlandgroove' ), esc_html__( '1 Comment', 'woodlandgroove' ), esc_html__( '% Comments', 'woodlandgroove' ) );
			echo '</span>';
		}

		edit_post_link( esc_html__( 'Edit', 'woodlandgroove' ), '<span class="edit-link">', '</span>' );
	}
endif;

if ( ! function_exists( 'woodlandgroove_post_navigation' ) ) :
	/**
	 * Lf Post Navigation
	 */
	function woodlandgroove_post_navigation() {
		?>
		<div class="nav-links">
		<?php
		global $wp_query;
...
if ( ! function_exists( 'woodlandgroove_share_links' ) ) :
	/**
	 * Lf Share links
	 **/

	function woodlandgroove_share_links() {
		?>
...
if ( ! function_exists( 'woodlandgroove_archive_title' ) ) :
	/**
	 * @param string $before_title
	 * @param string $after_title
	 * @return string
	 */
	function woodlandgroove_archive_title( $before_title = "", $after_title = "" ) {
